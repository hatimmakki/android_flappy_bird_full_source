...
public class SoundEngine1 {
    ...
    static SoundEngine1 _sharedEngine = null;

    public static SoundEngine1 sharedEngine() {
        synchronized(SoundEngine1.class) {
            if (_sharedEngine == null) {
                _sharedEngine = new SoundEngine1();
            }
        }
	...
	public void playEffectwithloop(Context app, int resId,int loop) {
		Integer sndId = -1;
		synchronized (effectsMap) {
			sndId = effectsMap.get(resId);
			if (sndId == null) {
		...
		int streamId = sp.play(sndId, 1.0f, 1.0f, 1, loop, 1.0f);
		if (effectsVolume != null) {
			sp.setVolume(streamId, effectsVolume, effectsVolume);
		}
		streamsMap.put(resId, streamId);
	...
	{
		this.stopEffect(m_context, m_Id);
	}
